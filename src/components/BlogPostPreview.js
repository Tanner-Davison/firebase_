...
const BlogPostPreview = ({ content }) => {
  ...
  return (
    <Wrapper>
      ...
      {content?.blogImageUrl && <BlogPhoto src={content.blogImageUrl} alt='blog-ref' />}
      {content?.authored && <Author>By: {content.authored}</Author>}
      </ImageAndDateDiv>
      <TextContentDiv>
      {content?.blogText  && <Body className={'text'}>{TruncatedText({text:content.blogText, maxLength:85})}</Body>}
      </TextContentDiv>
